...
export default function PaymentOptionForm({ coins, networks, onSubmit, disabled }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCoinId('');
      setNetworkId('');
      setMsg('Payment option ditambahkan');
      setTimeout(() => setMsg(null), 2000);
    } catch (err: any) {
      setMsg(err?.message || 'Gagal menambah payment option');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border p-4 space-y-3">
      <h2 className="font-medium">Tambah Payment Option</h2>
